...
/**
 * 查询当前数据库就绪状态
 * 仅当连接成功 + 全部集合的 JSON Schema 校验器就位 + 索引建立后返回 true
 */
...
export async function setupDatabase(mongodbUri, logger) {
  ...
  // 4. 建立或校验 comics 集合的 JSON Schema 校验器
  try {
    await ensureCollectionSchema(COMIC_COLLECTION, COMIC_JSON_SCHEMA, Comic, logger);
    logger.info({ event: "comics_schema_validator_ensured" });
  } catch (err) {
    logger.error({ event: "comics_schema_validator_failed", error: err.message });
    return;
  }

  // 5. 建立或校验 image_assets 集合的 JSON Schema 校验器
  try {
    await ensureCollectionSchema(IMAGE_ASSET_COLLECTION, IMAGE_ASSET_JSON_SCHEMA, ImageAsset, logger);
    logger.info({ event: "image_assets_schema_validator_ensured" });
  } catch (err) {
    logger.error({ event: "image_assets_schema_validator_failed", error: err.message });
    return;
  }
