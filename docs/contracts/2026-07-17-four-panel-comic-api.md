...
| 方法与路径 | 请求字段 | 关键响应字段 | 规则 |
| --- | --- | --- | --- |
| `GET /admin/comments` | `cursor?`、`limit?`、`view=active|deleted`；默认 `active`，`limit` 为 1 至 50。 | `{ items, nextCursor }`；单项为 `{ id, content, createdAt, author, episode, audit }`。 | 需要管理员会话；按 `createdAt`、`_id` 倒序游标分页。 |
| `DELETE /admin/comments/:commentId` | 无。 | `deleted:true`。 | 需要管理员会话；逻辑删除任意评论，写入 `deletedAt`、`deletedByRole:"admin"`、`deletedById`，不受单话发布状态限制。 |
...
评论列表中的 `author` 为 `{ id:string|null, displayName:string|null, avatarUrl:string|null }`，`episode` 为 `{ id:string|null, title:string|null, status:string|null }`；关联记录已不存在时保留可用原 ID，无法取得时为 `null`。`audit` 为 `null` 或 `{ deletedAt, deletedBy:{ role, id } }`。评论接口的非法查询参数或评论 ID 返回 `400 VALIDATION_ERROR`；缺失、失效或非管理员会话返回 `401 ADMIN_AUTH_REQUIRED`；评论不存在返回 `404 RESOURCE_NOT_FOUND`；重复删除返回 `409 COMMENT_ALREADY_DELETED` 并保留首次审计记录。
